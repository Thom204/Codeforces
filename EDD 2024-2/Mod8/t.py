...
class AVLTree(object):
    ...
    def findLevel(self, key):
        return self._findLevelRecursively(self.root, key, 0)

    def _findLevelRecursively(self, root, key, level):
        if root is None:
            return -1 
        if root.key == key:
            return level
        elif key < root.key:
            return self._findLevelRecursively(root.left, key, level + 1)
        else:
            return self._findLevelRecursively(root.right, key, level + 1)
